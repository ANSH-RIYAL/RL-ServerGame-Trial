...
def displayfield(field):
    ...
    image = np.zeros((70*15, 70*20, 3), dtype = np.uint8)
    field = np.asarray(field)
    for i in range(field.shape[0]):
        for j in range(field.shape[1]):
            if field[i][j] == 1:
                image = overWrite(15*i,20*j,image,otherPlayer)
            elif field[i][j] == 2:
                image = overWrite(15*i,20*j,image,smallFruit)
            elif field[i][j] == 3:
                image = overWrite(15*i,20*j,image,bigFruit)
...
otherPlayer = cv2.resize(otherPlayer, (15,20))
...
smallFruit = cv2.resize(smallFruit, (15,20))
...
bigFruit = cv2.resize(bigFruit, (15,20))
